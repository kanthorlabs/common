...
func TestMap(t *testing.T) {
	...
	for i := 0; i < counter; i++ {
		index := i
		wg.Go(func() {
			m.Set(fmt.Sprintf("index_%d", index), testdata.ErrorGeneric)
		})
	}
	wg.Wait()
	...
	require.True(t, has)
	require.ErrorIs(t, testdata.ErrorGeneric, v)

	require.ErrorIs(t, testdata.ErrorGeneric, m.Sample())
	require.Equal(t, counter, m.Count())
	require.Equal(t, counter, len(m.Data()))
	require.Equal(t, counter, len(m.Keys()))

	m.Merge(map[string]error{"new": testdata.ErrorGeneric})
